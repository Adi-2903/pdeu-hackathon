...
import { getJobs, getShortlistByJob } from "@/frontend/lib/db/queries";
...
interface ShortlistPageProps {
  initialJobs: { id: string; title: string; status: string }[];
  initialShortlist: any[];
  initialJobId: string;
}

const supabase = createClient();

export const getServerSideProps: GetServerSideProps<ShortlistPageProps> = async () => {
  const jobs = await getJobs();
  const defaultJobId =
    jobs.find((j) => j.id === "job-001")?.id ?? (jobs[0]?.id ?? "");

  const initialShortlist = defaultJobId ? await getShortlistByJob(defaultJobId) : [];

  return {
    props: {
      initialJobs: jobs,
      initialShortlist,
      initialJobId: defaultJobId,
    },
  };
};
...
export default function ShortlistPage({
  ...
  const [dbShortlist, setDbShortlist] = useState<any[]>(initialShortlist);
  ...
  const loadShortlistForJob = async (jobId: string) => {
    ...
    const { data } = await supabase
      .from("shortlists")
      .select("*, candidates(id, full_name, location, skills, sources)")
      .eq("job_id", jobId)
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-6 items-start">
      ...
      <Card className="bg-white rounded-xl border-gray-200 shadow-sm sticky top-6">
        ...
        <CardContent className="space-y-6">
          <div className="space-y-2">
            ...
            <Select value={selectedJobId} onValueChange={loadShortlistForJob}>
      ...
      <Card className="bg-white rounded-xl border-gray-200 shadow-sm min-h-[600px]">
        ...
        <CardContent className="p-0">
          {/* Database-backed shortlist for the selected job */}
          <div className="divide-y divide-gray-50">
            {dbShortlist.map((item, index) => {
              const candidate = item.candidates;
              const score = item.score ?? 0;
              const reason = item.notes as string | null;
              ...
              return (
                <div
                  ...
                  className="p-6 flex flex-col md:flex-row items-start gap-6 hover:bg-orange-50/10 transition-colors"
                >
                  <div className="flex items-center gap-4 shrink-0">
                    <span className="text-lg font-black text-orange-500 w-6 italic">
                      #{index + 1}
                    </span>
                  ...
                  <div className="flex-1 space-y-3 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(candidate?.skills || []).slice(0, 3).map((skill: string) => (
                        <Badge
                          key={skill}
                          className="bg-orange-50 text-orange-600 border border-orange-100 rounded-lg text-[10px] font-bold px-2 py-0"
                        >
                          {skill}
                        </Badge>
                      ))}
                  ...
                  <div className="flex flex-row md:flex-col items-center justify-between md:justify-center gap-4 w-full md:w-32 shrink-0 md:border-l border-gray-50 md:pl-6">
                    <div
                      className={`px-4 py-2 rounded-2xl border text-center w-full md:w-auto ${getScoreColor(
                        score
                      )}`}
                    >
          ...
          {!results && !isScoring ? (
            <div className="flex flex-col items-center justify-center py-16 text-center space-y-4 px-10">
              ...
              <div className="space-y-1">
                <h3 className="text-lg font-bold text-gray-800">Ready to rescore?</h3>
                <p className="text-sm text-gray-400 max-w-xs font-medium">
                  Paste a job description on the left to see updated AI scores for your talent pool.
                </p>
              </div>
            </div>
          ) : isScoring ? (
