...
import { ArrowLeft, HardDrive, Users, Send, Briefcase } from "lucide-react";
...
export default function JobDetailPage() {
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      {/* NAVIGATION */}
      <Link href="/jobs" className="text-gray-500 hover:text-[#F97316] flex items-center gap-2 transition-colors group font-medium text-sm">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-gray-500 font-medium lowercase">Managing role: <span className="font-mono text-[#F97316] uppercase">{id || "UNKNOWN"}</span></p>
        </div>
      ...
      <Card className="bg-white rounded-3xl border-gray-200 shadow-sm overflow-hidden">
        <CardHeader className="border-b border-gray-50 pb-6 bg-gradient-to-r from-white to-orange-50/20">
          <div className="flex items-center gap-4">
            ...
            <div>
              <CardTitle className="text-lg font-bold text-gray-900">Pipeline Intelligence</CardTitle>
              <CardDescription className="text-gray-500 font-medium">Candidate matching coming soon</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="py-24 text-center space-y-6">
          <div className="bg-orange-50 h-24 w-24 rounded-full flex items-center justify-center mx-auto shadow-sm group">
            <HardDrive className="h-10 w-10 text-orange-200 group-hover:text-[#F97316] transition-all group-hover:scale-110" />
          </div>
          <div className="space-y-2">
            <h3 className="text-xl font-bold text-gray-900">Deep Pipeline Exploration</h3>
            <p className="text-gray-500 max-w-sm mx-auto font-medium leading-relaxed">
              Matching candidates against specific job requirements and managing multi-stage technical pipelines will be implemented in Phase 5.
            </p>
          </div>
        </CardContent>
      </Card>
